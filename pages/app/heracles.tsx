...
import Heracles from "../../components/heracles";
            ...
            <div className="postIts">
                {categories.map((c, k) =>
                    <div className="categoryTodo" key={k}>
                        <h2>{c.name} <span onClick={() => deleteCategory(c.id as number)}>X</span></h2>
                        {c.todos?.map((t, kk) =>
                            <div key={kk} className="task_container" >
                                <Checkbox checked={t.completed} onChange={() => updateTodo(t)} label={t.task} className={t.completed ? "task checked" : "task"} />
                                <span onClick={() => deleteTodo(t)}>X</span>
                            </div>
                        )}
                    </div>
                )}
